...
function validateAndFixQuestion(parsed, topic, language, difficulty, randomData = null) {
  ...
  } else if (!fixed.exampleOutput || fixed.exampleOutput === "// Example output" || fixed.exampleOutput.includes("compute")) {
  ...
  if (category === 'OOP' && fixed.exampleInput && fixed.exampleInput.startsWith('[')) {
    const oopData = generateOOPExample(topic, language, difficulty);
  ...
  if (category === 'ARRAY' && fixed.problemStatement) {
    ...
    let inputArr;
    try { inputArr = JSON.parse(fixed.exampleInput); } catch (e) { /* bỏ qua */ }

    if (Array.isArray(inputArr) && inputArr.length > 0) {
      // 1. findSubarrayWithMaxSum - trả về mảng con
      if (fn.includes('findsubarraywithmaxsum') ||
      ...
      else if (fn.includes('maxsubarraysum') ||
      ...
      // 3. findSubarraySums - trả về mảng tất cả tổng
               ...
               (ps.includes('subarray') && ps.includes('sum') && !ps.includes('maximum') && !ps.includes('max'))) {
      ...
      else if (fn.includes('findsecondmaxandmin') ||
      ...
      else if (fn.includes('findsecondmax') && !fn.includes('findsecondmaxandmin')) {
      ...
      if (fn.includes('findsubarraywithmaxsum') ||
        ...
        fixed.expectedType = 'number';
                 ...
                 (ps.includes('subarray') && ps.includes('sum') && !ps.includes('maximum'))) {
        fixed.exampleOutput = '[]';
        fixed.expectedType = 'array';
      } else if (fn.includes('findsecondmax') || fn.includes('findsecondmin') || fn.includes('findsecondmaxandmin')) {
        fixed.exampleOutput = JSON.stringify([-1]);
        fixed.expectedType = 'array';
      }
    }
