...
function generateDetailedMCQExplanation(
  ...
  const explanations = {
    ...
    "How do you handle routing in Express.js?":
      ...
      `"${userAnswer}" is ${
        userAnswer === "Using Express Router"
          ? "correct"
          : userAnswer === "Using React Router"
            ? "React Router is for client-side routing"
    ...
    "What is the difference between a controlled and uncontrolled component in React?":
      ...
      `"${userAnswer}" is ${
        userAnswer === "Controlled components use state, uncontrolled use refs"
          ? "correct"
          : "incorrect"
      }.`,
...
function buildEssayResult(q, userAnswer, aiData) {
  ...
  if (finalScore >= 9) {
    detailedFeedback = `🌟 Excellent! Your answer is nearly perfect.${strengthText}${weaknessText}`;
  } else if (finalScore >= 7) {
    detailedFeedback = `👍 Good job! You demonstrated a strong understanding of most required concepts.${strengthText}${weaknessText}`;
  } else if (finalScore >= 5) {
    detailedFeedback = `📚 Satisfactory. Your answer meets the basic requirements but needs more detail to achieve a higher score.${strengthText}${weaknessText}`;
  } else if (finalScore >= 3) {
    detailedFeedback = `⚠️ Needs Improvement. Your answer is missing several important points.${strengthText}${weaknessText}`;
  } else {
    detailedFeedback = `❌ Your answer does not adequately address the question or is too brief.${strengthText}${weaknessText}`;
  }
