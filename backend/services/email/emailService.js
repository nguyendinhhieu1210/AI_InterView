...
const { createTransporter, verifyConnection } = require('../../config/emailConfig');
 ...
 */
const sendInterviewResultEmail = async (userId, interviewType, interviewData) => {
  try {
    ...
    const user = await User.findById(userId).select('userName email emailPreferences');
    ...
    if (user.emailPreferences && user.emailPreferences.interviewResults === false) {
    ...
    const htmlContent = getTemplateByType(interviewType, user.userName, interviewData);
...
Email tự động từ hệ thống phỏng vấn. Vui lòng không trả lời email này.
    ...
    console.log(`✅ Email sent to ${user.email} - Type: ${interviewType} - MessageID: ${info.messageId}`);
    return true;
    
  } catch (error) {
    console.error(`❌ Error sending ${interviewType} interview email:`, error.message);
