// services/ai/groqService.js - FIXED
...
class GroqService {
  ...
  async invokeWithRetry(messages, retryCount = 1) {
    try {
      ...
      // ===== FIX: Trả về object có cả content và usage =====
      return {
  ...
  async invokeWithRetryV2(messages, retryCount = 1) {
    const result = await this.invokeWithRetry(messages, retryCount);
    return result; // { content: string, usage: object }
  }
