      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
        <div className="flex flex-wrap justify-between items-center gap-3 mb-6">
          <button
            ...
            className="group flex items-center gap-2 text-blue-600 dark:text-blue-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-300 hover:gap-3 font-medium bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm"
          >
            <ArrowLeft className="w-5 h-5 transition-transform group-hover:-translate-x-1" />
            Back
          </button>
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full px-5 py-2 shadow-sm">
            <BarChart3 className="w-4 h-4 inline mr-2 text-blue-500" />
            <span className="font-semibold">{history.length} Interview sessions</span>
          </div>
        </div>

        <div className="mb-10 text-center sm:text-left">
          <h1 className="text-3xl sm:text-4xl font-extrabold pb-2 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent">
            Interview History
          </h1>
        ...
        {history.length === 0 ? (
          <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur rounded-2xl p-12 text-center">
            ...
            <button onClick={() => navigate('/welcome')} className="mt-4 px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700">Start your first interview →</button>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedHistory.map((item) => (
                <div key={item.id} className="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border-l-8 border-l-blue-500 overflow-hidden flex flex-col">
                  <div className="p-5 flex-1">
                    <div className="flex flex-wrap justify-between items-start gap-2 mb-3">
                      <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/70 text-blue-800 dark:text-blue-200 text-sm font-bold rounded-full">
                        {item.topic}
                      </span>
                      <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-xs rounded-full">{item.difficulty}</span>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mb-3">
                      <Calendar className="w-3.5 h-3.5" /> {formatDate(item.createdAt)}
                      <HelpCircle className="w-3.5 h-3.5 ml-1" /> {item.totalQuestions} questions
                    </div>
                    ...
                    <div className="flex justify-end items-baseline gap-1 mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <span className={`text-2xl font-bold ${getScoreColor(item.totalScore)}`}>{item.totalScore}<span className="text-sm">/100</span></span>
                    </div>
                  </div>
                  <div className="p-4 pt-0">
                    <button onClick={() => navigate(item.detailPath)} className="w-full flex items-center justify-center gap-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 py-2 rounded-xl transition">
            ...
            {totalPages > 1 && (
              <div className="flex justify-center gap-2 mt-12">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <button key={p} onClick={() => handlePageChange(p)} className={`w-9 h-9 rounded-full ${currentPage === p ? 'bg-blue-600 text-white' : 'bg-white/70 dark:bg-gray-800/70 hover:bg-blue-100'}`}>{p}</button>
                ))}
