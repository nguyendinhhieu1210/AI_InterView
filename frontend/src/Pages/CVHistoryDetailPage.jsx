...
export default function CVHistoryDetailPage() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-bg">
                <div className="text-center">
                    ...
                    <p className="text-muted font-medium animate-pulse leading-relaxed">Loading CV session...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-4 bg-bg">
                <div className="bg-card backdrop-blur rounded-2xl p-8 text-center shadow-soft border border-border max-w-md w-full">
                    ...
                    <p className="text-text mb-6 leading-relaxed">{error || 'CV session not found'}</p>
                    <button onClick={() => navigate('/cv-history')} className="px-5 py-2.5 bg-primary text-white rounded-xl hover:brightness-105 transition shadow-md">
                        Back
                    </button>
                </div>
            </div>
    ...
    // Tone màu: teal (giống CVHistoryPage)
    const cvColor = {
        primary: 'teal',
        borderMcq: 'border-l-teal-500',
        borderEssay: 'border-l-cyan-500',
                    ...
                    {skillTags && skillTags.length > 0 && (
                        <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-3">
                            {skillTags.map((tag, idx) => (
                                <span key={idx} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-teal-100/80 dark:bg-teal-950/50 text-teal-800 dark:text-teal-200 text-xs rounded-full font-medium shadow-sm leading-relaxed">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-12">
                    <div className="group bg-card/80 backdrop-blur-sm rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className={`text-3xl font-bold ${getScoreColor(totalScore)} leading-none`}>
                            {totalScore}<span className="text-sm text-muted">/{maxPossibleScore}</span>
                        </div>
                        <div className="text-xs text-muted mt-2 font-medium">Total Score</div>
                    </div>
                    ...
                    <div className="group bg-card/80 backdrop-blur-sm rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className="text-3xl font-bold text-cyan-600 dark:text-cyan-400 leading-none">{totalQuestions}</div>
                        <div className="text-xs text-muted mt-2 font-medium">Total Questions</div>
                    </div>
                    ...
                    <div className="group bg-card/80 backdrop-blur-sm rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className="text-3xl font-bold text-success leading-none">{results.filter(r => r.isCorrect === true).length}</div>
                        <div className="text-xs text-muted mt-2 font-medium">Correct Answers</div>
                    </div>
                    ...
                    <div className="group bg-card/80 backdrop-blur-sm rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className="text-3xl font-bold text-primary leading-none">{Math.round((totalScore / maxPossibleScore) * 100)}%</div>
                                    ...
                                    <div className="px-5 pb-6 space-y-5 animate-slideDown border-t border-border pt-5">
                                        <div>
                                            <p className="font-semibold text-text mb-2 flex items-center gap-1 text-sm leading-relaxed">
                                                <span className="w-1.5 h-1.5 rounded-full bg-cyan-500"></span> Your Answer
                                            </p>
                                        ...
                                        {result.strengths?.length > 0 && (
                                            <div>
                                                <p className="font-semibold text-success mb-2 text-sm leading-relaxed">✅ Strengths</p>
                                                <ul className="space-y-2">
                                                    {result.strengths.map((s, i) => (
                                                        <li key={i} className="bg-success/5 border border-success/20 rounded-xl px-3 py-2 text-sm text-muted flex items-start gap-1 leading-relaxed">
                                        ...
                                        {result.mistakes?.length > 0 && (
                                            <div>
                                                <p className="font-semibold text-error mb-2 text-sm leading-relaxed">❌ Mistakes</p>
                                                <ul className="space-y-2">
                                                    {result.mistakes.map((m, i) => (
                                                        <li key={i} className="bg-error/5 border border-error/20 rounded-xl px-3 py-2 text-sm text-muted flex items-start gap-1 leading-relaxed">
                                        ...
                                        {result.idealAnswerKeywords?.length > 0 && (
                                            <div>
                                                <p className="font-semibold text-muted mb-2 text-sm leading-relaxed">🔑 Important Keywords</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {result.idealAnswerKeywords.map((k, i) => (
                                                        <span key={i} className="px-3 py-1.5 rounded-full bg-primary/10 text-primary text-xs font-medium shadow-sm leading-relaxed">
                                                            {k}
                                                        </span>
                                                    ))}
                                                </div>
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-2">
                                        {summary.strengths.map((s, i) => (
                                            <li key={i} className="flex items-start gap-2 text-sm text-muted bg-success/5 p-3 rounded-xl leading-relaxed">
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-2">
                                        {summary.weaknesses.map((w, i) => (
                                            <li key={i} className="flex items-start gap-2 text-sm text-muted bg-warning/5 p-3 rounded-xl leading-relaxed">
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-2">
                                        {summary.suggestions.map((sug, i) => (
                                            <li key={i} className="flex items-start gap-2 text-sm text-muted bg-primary/5 p-3 rounded-xl leading-relaxed">
