        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-4 bg-bg">
                <div className="bg-card rounded-2xl p-8 text-center max-w-md shadow-soft border border-border">
                    ...
                    <button onClick={fetchSessionDetail} className="px-5 py-2.5 bg-primary text-white rounded-xl flex items-center gap-2 mx-auto hover:brightness-105 transition shadow-md">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 md:py-10">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-semibold text-text">{session.language}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Layers className="w-4 h-4 text-muted" />
                                    <span className="text-muted">Domain:</span>
                                    <span className="font-semibold text-text">{session.domain}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Calendar className="w-4 h-4 text-muted" />
                                    <span className="text-muted">Date:</span>
                                    <span className="font-medium text-text">{formatDate(session.createdAt)}</span>
                                </div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-12">
                    <div className="group bg-card rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className={`text-3xl font-bold ${getScoreColor(totalScore, maxPossibleScore)} leading-none`}>
                    ...
                    <div className="group bg-card rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className="text-3xl font-bold text-success leading-none">{correctCount}</div>
                    ...
                    <div className="group bg-card rounded-2xl p-5 text-center shadow-soft hover:shadow-lg transition-all duration-300 border border-border hover:scale-105">
                        ...
                        <div className="text-3xl font-bold text-purple-600 dark:text-purple-400 leading-none">{accuracy}%</div>
                ...
                {codeHistory.length === 0 ? (
                    <div className="bg-card rounded-2xl p-12 text-center shadow-soft border border-border">
                        ...
                        <p className="text-muted">No code submissions in this session yet.</p>
                    </div>
                ) : (
                                    ...
                                    <div className="px-5 pb-6 space-y-5 animate-slideDown border-t border-border pt-5">
                                            ...
                                            <div>
                                                ...
                                                <div className="space-y-4">
                                                    {round.explainAnswers.map((qa, qIdx) => (
                                                        <div key={qIdx} className="border-l-4 border-primary pl-4 py-2">
                                                            <div className="flex items-center gap-2 mb-2">
                                                                ...
                                                                <span className="font-medium text-text">Question {qIdx + 1}</span>
                                                            </div>
                                                            <p className="text-muted mb-2 text-sm">{qa.question}</p>
                                                            <div className="bg-muted/5 p-3 rounded-lg mb-2 border border-border">
                                                                ...
                                                                <p className="text-text mt-1 text-sm">{qa.answer}</p>
                                                            </div>
                                                            {qa.feedback && (
                                                            ...
                                                            {qa.modelAnswer && (
                                                                <div className="mt-2 text-sm bg-primary/10 p-2 rounded text-muted">
                                                                    <span className="font-semibold text-primary">AI answer:</span> {qa.modelAnswer}
                                                                </div>
                                                            )}
                                                        </div>
                                                    ))}
                                        ...
                                        {round.evaluation && (
                                            <div className="mt-4 bg-primary/5 rounded-xl p-4 border border-primary/20">
                                                ...
                                                <p className="text-sm text-muted mb-3">{round.evaluation.summary}</p>
