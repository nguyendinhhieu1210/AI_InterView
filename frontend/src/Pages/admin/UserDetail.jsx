...
import { ArrowLeft, Mail, Calendar, Shield, CheckCircle, XCircle, Edit2, Save, X } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
              ...
              <div>
                ...
                {editing ? (
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-white capitalize">{user.role}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
              {user.isVerified ? <CheckCircle className="w-5 h-5 text-emerald-500" /> : <XCircle className="w-5 h-5 text-amber-500" />}
              <div>
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
            {editing ? (
              <>
                <button onClick={handleSave} className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg">
                  <Save size={16} />
                  Save Changes
                </button>
                <button onClick={() => setEditing(false)} className="inline-flex items-center gap-2 px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-lg">
                  <X size={16} />
                  Cancel
                </button>
              </>
            ) : (
              <button onClick={() => setEditing(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-500 text-white rounded-lg">
                <Edit2 size={16} />
                Edit Profile
              </button>
            )}
