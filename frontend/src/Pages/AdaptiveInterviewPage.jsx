      ...
      <div className="min-h-screen bg-bg flex items-center justify-center p-4">
        <div className="relative max-w-lg w-full">
          ...
          <div className="relative bg-card rounded-3xl shadow-soft border border-border p-8 transition-all duration-500">
            ...
            <h2 className="text-3xl font-bold text-center text-text mb-2">Adaptive Interview</h2>
            <div className="flex justify-center gap-3 mb-6">
              <span className="px-3 py-1 rounded-full bg-primary/20 text-primary text-sm font-medium shadow-sm">{topic}</span>
              <span className={`px-3 py-1 rounded-full font-medium text-sm shadow-sm ${getDifficultyBadgeClass()}`}>
            ...
            {!ready ? (
              <>
                <div className="bg-muted/5 rounded-2xl p-5 mb-8 border border-border">
                  <p className="font-semibold text-text flex items-center gap-2 mb-3">
                    <Sparkles className="w-5 h-5 text-primary" /> How the interview works:
                  </p>
                  <ul className="space-y-2 text-muted text-sm">
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-success mt-0.5 shrink-0" /><span>AI starts with an initial question tailored to your selected topic and difficulty</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-success mt-0.5 shrink-0" /><span>You answer naturally — AI analyzes your depth and adjusts subsequent questions</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-success mt-0.5 shrink-0" /><span>The conversation adapts in real-time, diving deeper where you excel</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-success mt-0.5 shrink-0" /><span>After 8 questions, you'll receive a detailed score and analysis</span></li>
                  </ul>
                </div>
                <button onClick={() => setReady(true)} className="w-full py-3.5 bg-primary hover:brightness-105 text-white rounded-xl font-semibold shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group">
              ...
              <div className="text-center py-6">
                ...
                <div className="mt-6">
                  <div className="text-6xl font-black text-text tabular-nums">{countdown}</div>
                  <p className="text-muted mt-2 font-medium">{countdown === 1 ? "Take a deep breath..." : "Get ready..."}</p>
                </div>
              </div>
            )}
          </div>
    ...
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <header className="sticky top-0 z-20 backdrop-blur-xl bg-card/80 border-b border-border shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
          <button onClick={() => navigate('/welcome')} className="group flex items-center gap-2 text-muted hover:text-primary transition-all duration-300 font-medium">
      ...
      {isAnalyzing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md animate-fadeIn">
          <div className="bg-card rounded-2xl p-8 shadow-soft flex flex-col items-center gap-4 max-w-sm mx-4 border border-border">
            ...
            <p className="text-lg font-semibold text-text">AI is analyzing your answers...</p>
        ...
        <div className="mt-8 mb-4 animate-fadeIn px-4">
          <div className="bg-card rounded-2xl p-6 shadow-soft border border-border">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-bold text-text mb-1">Interview Completed!</h3>
              <div className="text-5xl font-extrabold text-primary my-2">{finalScore}/10</div>
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                <button onClick={() => setShowReportModal(true)} className="px-5 py-2.5 bg-primary hover:brightness-105 text-white rounded-xl transition shadow-md flex items-center gap-2 font-medium">
                  <Award className="w-4 h-4" /> View Full Report
                </button>
                <button onClick={() => navigate('/history')} className="px-5 py-2.5 bg-secondary hover:brightness-105 text-white rounded-xl transition shadow-md flex items-center gap-2 font-medium">
                  <CheckCircle className="w-4 h-4" /> History
                </button>
                <button onClick={resetInterview} className="px-5 py-2.5 bg-card border border-border hover:bg-muted/10 text-text rounded-xl transition shadow-sm flex items-center gap-2 font-medium">
                  <RotateCw className="w-4 h-4" /> New Interview
                </button>
                <button onClick={() => navigate('/welcome')} className="px-5 py-2.5 bg-card border border-border hover:bg-muted/10 text-text rounded-xl transition shadow-sm flex items-center gap-2 font-medium">
