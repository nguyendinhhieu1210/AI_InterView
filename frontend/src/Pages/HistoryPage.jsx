...
export default function HistoryPage() {
  ...
  const themes = {
    ...
    cv: {
      bgIcon: 'bg-teal-100 dark:bg-teal-950/40',      // đổi từ emerald sang teal
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-bg">
        <div className="bg-card rounded-2xl p-8 text-center max-w-md shadow-soft border border-border">
          ...
          <button onClick={fetchCounts} className="px-5 py-2.5 bg-primary text-white rounded-xl flex items-center gap-2 mx-auto hover:brightness-105 transition-all shadow-md">
  ...
  return (
    <div className={`min-h-screen bg-bg transition-all duration-700 ${pageLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          {cards.map((card) => {
            ...
            return (
              <div
                ...
                className={`group cursor-pointer bg-card rounded-2xl border ${card.theme.border} shadow-soft hover:shadow-lg hover:-translate-y-2 transition-all duration-300 overflow-hidden transform-gpu flex flex-col`}
              >
                <div className="p-6 text-center flex flex-col items-center h-full">
                  ...
                  <h2 className="text-2xl font-bold text-text mb-2">{card.title}</h2>
