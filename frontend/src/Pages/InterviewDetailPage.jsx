    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-bg">
        <div className="bg-card backdrop-blur rounded-2xl p-8 text-center shadow-soft border border-border">
          ...
          <button onClick={() => navigate('/history')} className="px-4 py-2 bg-primary text-white rounded-xl hover:brightness-105 transition shadow-md">
            Back to History
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="bg-card/80 backdrop-blur rounded-2xl p-4 text-center shadow-soft border border-border hover:shadow-lg transition-all">
            ...
            <div className={`text-3xl font-bold ${getScoreColor(interview.totalScore)}`}>
              {interview.totalScore}<span className="text-base text-muted">/100</span>
            </div>
            <div className="text-xs text-muted mt-1">Total Score</div>
          </div>
          <div className="bg-card/80 backdrop-blur rounded-2xl p-4 text-center shadow-soft border border-border hover:shadow-lg transition-all">
            <div className="text-3xl font-bold text-success">
              {interview.mcqScore || 0}<span className="text-base text-muted">/{interview.mcqCount * 10}</span>
            </div>
            <div className="text-xs text-muted mt-1">MCQ Score</div>
          </div>
          <div className="bg-card/80 backdrop-blur rounded-2xl p-4 text-center shadow-soft border border-border hover:shadow-lg transition-all">
            <div className="text-3xl font-bold text-secondary">
              {interview.essayScore || 0}<span className="text-base text-muted">/{interview.essayCount * 10}</span>
            </div>
        ...
        <div className="space-y-4">
          ...
          {mcqResults.map((result, idx) => {
            ...
            return (
              <div key={`mcq-${idx}`} className="group bg-card backdrop-blur rounded-2xl border-l-8 border-l-primary shadow-soft hover:shadow-lg transition-all overflow-hidden">
                <button
                ...
                >
                  <div className="flex items-start gap-3">
                    {isCorrect ? <CheckCircle className="w-5 h-5 text-success mt-0.5 shrink-0" /> : <XCircle className="w-5 h-5 text-error mt-0.5 shrink-0" />}
                    <div>
                      <div className="font-medium text-text">Question {idx + 1}: {questionText}</div>
                  ...
                  {expandedQuestion === `mcq_${idx}` ? <ChevronUp className="w-5 text-muted shrink-0" /> : <ChevronDown className="w-5 text-muted shrink-0" />}
                </button>
                  ...
                  <div className="p-4 bg-muted/5 border-t border-border space-y-3 text-sm animate-slideDown">
                    ...
                    {explanation && (
                      <div className="mt-3 pt-2 border-t border-border">
                        <p className="font-semibold text-primary text-xs mb-1">Explanation:</p>
          ...
          {essayResults.map((result, idx) => {
            ...
            return (
              <div key={`essay-${idx}`} className="group bg-card backdrop-blur rounded-2xl border-l-8 border-l-secondary shadow-soft hover:shadow-lg transition-all overflow-hidden">
                <button
                ...
                >
                  ...
                  {expandedQuestion === `essay_${idx}` ? <ChevronUp className="w-5 text-muted" /> : <ChevronDown className="w-5 text-muted" />}
                </button>
                {expandedQuestion === `essay_${idx}` && (
                  <div className="p-4 bg-muted/5 border-t border-border space-y-3 text-sm animate-slideDown">
                    ...
                    {sampleAnswer && (
                      <div>
                        <p className="font-semibold text-text">Sample answer:</p>
                    ...
                    {idealKeywords.length > 0 && (
                      <div>
                        <p className="font-semibold text-text">Ideal keywords:</p>
                    ...
                    {explanation && (
                      <div>
                        <p className="font-semibold text-primary">Grading explanation:</p>
