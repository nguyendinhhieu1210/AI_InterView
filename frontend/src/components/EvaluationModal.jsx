              ...
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-card border border-border shadow-soft max-h-[92vh] flex flex-col">
                ...
                <div className="sticky top-0 z-10 px-6 py-4 border-b border-border bg-card">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      onClick={onClose}
                      className="p-1 rounded-lg transition hover:bg-muted/10 text-muted"
                    >
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                  ...
                  <div className="p-4 rounded-xl border border-primary/30 bg-primary/5">
                    <h4 className="font-semibold text-primary mb-2 text-base">🎯 Overall Assessment</h4>
                    <p className="text-text leading-relaxed">{evaluation.summary}</p>
                    {evaluation.feedback && (
                  ...
                  <div>
                    <h4 className="font-semibold text-primary mb-3">📝 Problem Statement</h4>
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {exampleInput && (
                        <div className="bg-muted/10 p-3 rounded-lg border border-border">
                          <p className="text-xs font-semibold text-muted mb-2 uppercase tracking-wide">📥 Example Input</p>
                      ...
                      {exampleOutput && (
                        <div className="bg-muted/10 p-3 rounded-lg border border-border">
                          <p className="text-xs font-semibold text-muted mb-2 uppercase tracking-wide">📤 Example Output</p>
                  ...
                  {code && (
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-semibold text-primary">💻 Your Submitted Code</h4>
                        <button
                  ...
                  {explainAnswers && explainAnswers.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-semibold text-warning text-base">💬 Explanation Q&A ({correctAnswers}/{totalAnswers})</h4>
                      ...
                      <div className="space-y-4">
                        {explainAnswers.map((item, idx) => (
                          ...
                          >
                            ...
                            <div className="mb-3">
                              <p className="text-sm font-medium text-text mb-1">Question:</p>
                              <p className="text-sm text-muted leading-relaxed">{item.question}</p>
                            </div>

                            <div className="mb-3">
                              <p className="text-sm font-medium text-text mb-1">Your Answer:</p>
                              <p className="text-sm text-muted leading-relaxed italic">{item.answer}</p>
                            </div>
                            ...
                            {item.modelAnswer && (
                              <div className="rounded-lg p-3 border border-primary/20 bg-primary/5">
                                <p className="text-xs font-semibold text-primary mb-2">🤖 Expected Answer (AI Model):</p>
                  ...
                  {evaluation.weaknesses && evaluation.weaknesses.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-error mb-3">⚠️ Areas to Improve</h4>
                      <ul className="space-y-2">
                ...
                <div className="sticky bottom-0 flex gap-3 p-6 border-t border-border bg-card">
                  <button
                    onClick={onClose}
                    className="flex-1 bg-muted/20 hover:bg-muted/30 text-text py-3 rounded-xl font-semibold transition duration-200 hover:shadow-md"
                  >
                    Close Report
                  </button>
